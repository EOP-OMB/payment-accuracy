SELECT
    ...
    a.[Fiscal_Year],
    a.[Payment_Type],
    b.Column_values AS [Overpayments_Within_Control_Why],
    ...
    a.[Financial],
    d.[Multiselect_Text] AS [Overpayment_Mitigations_Taken],
    e.[Multiselect_Text] AS [Overpayment_Mitigations_Planned],
    f.Column_values AS [Overpayments_Within_Control_Amount],
    g.[Multiselect_Text] AS [Overpayment_Combined_Mitigations_Taken],
    h.[Multiselect_Text] AS [Overpayment_Combined_Mitigations_Planned]
FROM (SELECT
    [Agency],
    [Program_Name],
    [Fiscal_Year],
...
LEFT JOIN (
    ...
    FROM (
        SELECT * FROM mitigation_strategies
        WHERE Column_names = 'cyp21_atp1_8'
        ORDER BY Multiselect_Text
    ) subquery1
    GROUP BY Agency, Fiscal_Year, Program_Name
) AS g
...
LEFT JOIN (
    ...
    FROM (
        SELECT * FROM mitigation_strategies
        WHERE Column_names = 'cyp21_app1_8'
        ORDER BY Multiselect_Text
    ) subquery2
    GROUP BY Agency, Fiscal_Year, Program_Name
) AS h
